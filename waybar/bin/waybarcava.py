...
def check_playing_playerctl():
    """
    audio detection that only returns True when there's actual media output.
    returns False during silence (like when Discord is connected but no one is speaking).
    """
    try:
        result = subprocess.run(
        ...
        detailed_result = subprocess.run(
            ["playerctl", "-p", "mpv", "status"], capture_output=True, text=True, timeout=2
        )

        if detailed_result.stdout == "No players found":
            return False
